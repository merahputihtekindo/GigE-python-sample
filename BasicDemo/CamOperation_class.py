...
class CameraOperation():
    ...
    def Work_thread(self,root,panel):
        ...
        while True:
            ...
            if 0 == ret:
                ...
                #Time start when the image is obtained
                self.st_frame_info = stOutFrame.stFrameInfo
                memmove(byref(buf_cache),stOutFrame.pBufAddr, self.st_frame_info.nFrameLen) #image buffer
                ...
                if True == self.b_save_jpg:
                    self.Save_jpg(buf_cache) # Save to JPG
                if True == self.b_save_bmp:
                    self.Save_Bmp(buf_cache) # Save to Bmp
            else:
            ...
            # if color not in RGB chanel, it will converted to RGB
            else:
            ...
            # catch buffer image to tkinter image
    ...
    def Save_Bmp(self,buf_cache):
        ...
        stParam = MV_SAVE_IMAGE_PARAM_EX()
        stParam.enImageType = MV_Image_Bmp;                                                # Image format to save
        stParam.enPixelType = self.st_frame_info.enPixelType                               # Camera pixel type
        stParam.nWidth      = self.st_frame_info.nWidth                                    # Width
        stParam.nHeight     = self.st_frame_info.nHeight                                   # Height
        stParam.nDataLen    = self.st_frame_info.nFrameLen
        ...
        img_buff = (c_ubyte * stParam.nImageLen)()
        try:
            memmove((img_buff), stParam.pImageBuffer, stParam.nImageLen)
            file_open.write(img_buff)
            self.b_save_bmp = False
